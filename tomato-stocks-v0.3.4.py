    ...
    if user_input == ("shop"):
        ...
        while user_in_shop == True:
            ...
            if user_input == ("buy tomato seeds"):
                print("Processing...")
                if money >= 1:
                    max_buy_tomatoes = money // 1
                    if max_buy_tomatoes > 1:
                        try:
                            qty = int(input(f"How many tomato seeds do you want to buy? (max {max_buy_tomatoes}): "))
                            if 1 <= qty <= max_buy_tomatoes:
                                money -= qty * 1
                                tomato_seeds += qty
                                print(f"Bought {qty} tomato seeds, you now have ${money} remaining.")
                            else:
                                print(f"Invalid quantity. Must be between 1 and {max_buy_tomatoes}.")
                        except ValueError:
            ...
            if user_input == ("buy fertilizer"):
                ...
                if fertilizer_stock > 0:
                    if money >= 10:
                        max_buy_fertilizer = min(money // 10, fertilizer_stock)
                        if max_buy_fertilizer > 1:
                            try:
                                qty = int(input(f"How many fertilizer do you want to buy? (max: {max_buy_fertilizer}): "))
                                if 1 <= qty <= max_buy_fertilizer:
                                    money -= qty * 10
                                    fertilizer_quantity += qty
                                    fertilizer_stock -= qty
                                    print(f"Bought {qty} fertilizer, you now have ${money} remaining.")
                                else:
                                    print(f"Invalid quantity. Must be between 1 and {max_buy_fertilizer}.")
                            except ValueError:
                                print("Please enter a valid number.")
                        else:
                            money -= 10
...
# script for debug
    if user_input == "show_debug":
        ...
        print(f"Max buy tomatoes: {max_buy_tomatoes}")
        print(f"Max buy fertilizer: {max_buy_fertilizer}")
